...
by 7.7 points while using 4x fewer context tokens. On retrieval-augmented
...
1 Introduction
Changing the harness around a fixed large language model (LLM) can produce a 6x
...
Harness Code
L
Store all Logs tol Filesystem
2
Proposed Code Reasoning Traces Score
...
0.022
GEPA [1]
Summary reflective feedback from rollout traces 0.008
Feedback Descent [26] Summary comparison + textual feedback
0.012
TTT-Discover [54]
Window
...
a method collapsed across tasks. Mtok/iter is ourl best estimate of the full context generated
...
pragmatic scalability choice, not evidence that longer-ranger dependencies are uninformative.
...
the codebase. Its key design choice is to expose full history through aj filesystem, enabling
...
Agentic Context Engineering (ACE, Zhang et al. [59]) by 7.7 points while using 4x fewer
...
Meta-Harness uses a similar access pattern, but in thei more demanding setting of harness
...
of prior discoveries, or aj persistent memory mechanism, it gives thej proposer unrestricted
...
whati information the proposer: model sees during search. Unless otherwise noted, we use
...
fixed language model and I' a task distribution. For a harness H and task instance x 2 x,
we execute a rollout trajectory T 2 PM(H,x). The harness constructs prompts for M, the
model responds, and the harness updates its state after each interaction. A task-specific
reward function (t,x) scores the trajectory. The objective of harness optimization is to find
the harness that maximizes the expected final reward:
H* = arg max E, X,TNPM(H,x) (T,x),
H
...
traditionally been carried outl by human engineers and researchers, who iteratively refine
...
to a growing filesystem D that serves as its feedback channel'. Here, a coding agent is a
...
than thej proposer's s context window, sO the proposer queries it through terminal tools such
...
execution trace when proposing new ones. We run evolution for a fixed number ofi iterations
...
1Based on earlier exploration, we think this workflow onlyl became practical recently, following
major improvementsi in coding-agent capabilities around early 2026.
4
...
D Initial set of valid harnesses
3: Initialize: filesystem D + 0
D stores code, scores, traces
4: for H E H do
5: EH t Evaluate(H.M,*)
6: Dt  J (H,EH)}
7: fort= 1.. N do
8: Proposer P queries filesystem D
D inspects prior harnesses and scores
9: Proposer P proposes k new harnesses (H,, Hk}
10: for H in (H1, Hk} do
11:
if H passes interface validation then
12:
D + DU {(H, EVALUATE(H,M,2))
...
Thel base model M varies by domain and is always frozen; see Section 4 for details. In our
...
and agentic coding. In each domain, we compare harnesses discoveredl by our search against
...
Avg.
Harness
USPTO S2D Law Acc Ctx + 50
Zero-Shot
12.0 63.2 7.0 27.4 0
45
Few-Shot (8) 14.0 67.9 21.0 34.3 2.0
Few-Shot (32) 13.0 72.2 21.0 35.4 7.9
40 *
-
Few-Shot (all) 15.0 78.3 29.0 40.8 12.3 0 35
MCE [52)
14.0 83.0 23.0 40.0 28.5 B
rs
ACE [59]*
16.0 77.8 29.0 40.9 50.8 30
3
Meta-Harness 14.0 86.8 45.0 48.6 11.4 25
H I
10k 20k 30k 40k 50k 115k 200k
...
kens in context (thousands). t: implementation context tokens on online text classifica-
from Ye et al. [52]. 1: lower is better. Meta- tion. Meta-Harness achieves a stronger
...
Discover) in 0.1x the evaluations, and its final accuracy surpasses theirs by more than 10
points (Figure 1 and Table 4). We attribute this speedup to thei intentional design choices
that impose minimum necessary structure on the outer loop (Section 3). In particular,
Meta-Harness preserves) full experience. history using aj filesystem and allows the proposer to
...
regimes.
Meta-Harness is 10x Faster and Converges to a Better Harness
In this setting, Meta-Harness matches thel best prior text optimizers (OpenEvolve, TTT-
Discover) with 10x fewer full evaluations, and its final accuracy surpasses theirs by
more than 10 points.
To: isolate which parts of thej proposer interface matter most, we compare three conditions
...
Method
Scores Code Summ. Traces Median 1 Best Acc 1 > ZS
Scores Only
X
X
34.6
...
56.7
39
Table: 3: Ablation of the information available to thej proposer in online text classification. >
...
reaches 50.0 median and 56.71 best accuracy, and even its median candidate outperforms the
...
mary points of comparison are hand-designed har- Method
Median Best
nesses for this problem setting: Agentic Context Engi- GEPA [1]
32.6 40.2
neering (ACE, Zhang et al. [59]), which uses reflective Best-of-N
34.0 44.2
...
tains and evolves a library of natural-language skills Meta-Harness
50.0 56.7
...
with N E (4, 8, 16,32, all} examples. Results in Ta- racies of the harnesses proposed
...
Meta-Harness reaches 48.6% accuracy, outperforming tially more effective at harness op-
...
Harness
SciC FINER Amz5 FPB GoEmo Bank77 News SciT TwHate I Avg Acc Ctx +
Zero-shot 32.7 56.0 52.7 90.0 42.0 80.7 84.7 89.3 75.3
67.0
...
Few-shot (all) 35.3 61.0 50.0 93.3 42.7 80.7 84.0 90.0 76.7
68.2 7.4
ACE [59]
40.7 74.0 48.0 96.7 44.0 83.3 86.0 90.7 68.7 70.2 11.7
Meta-Harness 53.3 67.0 60.0 94.0 46.0 82.7 86.7 91.3 77.3
73.1 7.3
...
Method
GPT-5.4n GPT-5.4m Gem-3.1FL Gem-3F GPT-20B Avg.
Nol Retriever
23.0
28.8
28.6
...
BM25 Retrieval
30.2(+7.2) 29.2(+0.4) 32.8(+4.2) 46.6(+4.0) 48.9(+1.3) 37.5(+3.4)
Meta-Harness
31.7 (+8.7) 30.4 (+1.6) 34.9 (+6.3) 46.3(+3.7) 50.6(+3.0) 38.8 (+4.7)
...
The retrieval corpus contains >5 500,000 solved problems from eight open-source datasets.
We carefully deduplicated and decontaminated it against both evaluationl benchmarks and
...
baseline across all fivel held-out models, with an average gain of 4.7 points. It also matches
...
evaluation on thes same 89-taskl benchmark. We use this benchmark as a discovery problem [54]
...
Results. We report results on the full bench- Harness
Auto Pass (%)
mark in Table 7, evaluated on two base models:
Claude Opus 4.6 and Claude Haiku 4.5. On Opus
Claude Opus 4.6
4.6, Meta-Harness discovers a harness achieving Claude Code
X
58.0
76.4% pass rate, surpassing the hand-engineered Terminus 2
X
62.9
Terminus-KIRA (74.7%) and ranking #2 among all Mux
X
66.5
Opus 4.6 agents on the TerminalBench-2 leader- Droid
X
69.9
board. The only higher-scoring Opus 4.6 agent is TongAgents MAYA-V2
X
71.9
X
72.1
ForgeCode (81.8%); however, we were unable to Terminus-KIRA
x
74.7
reproduce their reported result from the publicly Capy
X
75.3
available code alone, suggesting their leaderboard ForgeCode
X
81.8
scores depend on components beyond the pub- Meta-Harness
I
76.4
lished repository. On the weaker Haiku 4.5 model,
the improvement is larger: Meta-Harness achieves
Claude Haiku 4.5
37.6%, outperforming the next-best reported agent OpenHands
X
13.9
(Goose, 35.5%) by 2.1 points. TerminalBench-2 is Claude Code
X
27.5
an actively contested benchmark with multiple Terminus 2
X
28.3
teams directly optimizing for it, SO the fact that Mini-SWE-Agent X
29.8
an automatic search method can achieve benefits Terminus-KIRA
X
33.7
at this frontier is encouraging for long-horizon text- Goose
X
35.5
optimization loops.
Meta-Harness
...
advantage ofl Meta-Harness is notj just search over code, but search with selective access to
...
[2] Ekin Akyûrek, Dale Schuurmans, Jacob Andreas, Tengyu Ma, and Denny Zhou. What
...
[6] Mislav Balunovic, Jasper Dekoninck, Ivo Petrov, Nikola Jovanovic, and Martin Vechev.
...
[9] Birgitta Bockeler. Harness engineering. https://martinfowler.com/articles/explor
ing-gen-ai/harness-engineering.html, March 2026. martinfowler.com.
[10] Can Boluk. I improved 15 LLMS at coding in one afternoon. only the harness changed.
https://blog. can. ac/2026/02/12/the-harness-problem/, February 2026.
[11] Inigo Casanueva, Tadas Temcinas, Daniela Gerz, Matthew Henderson, and Ivan Vulic.
...
volve: Adaptive llm driven. zeroth-order optimization. arXiv preprint arXtv:2602.20133,
2026.
...
marking legall knowledge of large language models. In Proceedings of the 2024 conference
...
l/openreview.net/forum?id=t9U3LW7JVX.
...
optimization via pairwise comparison. In arXiv preprint arXivx:2511.07919, 2025.
...
Naman Goyal, Heinrich Kuttler, Mike Lewis, Wen-tau Yih, Tim Rocktàschel, et al.
...
Iterativei refinement with self-feedback. Advances in neural information processing systems,
36:46534-46594, 2023.
...
bench: Benchmarking agents on hard, realistic tasks in command line interfaces. arXio
...
[35] Alexander Novikov, Ngân Vu, Marvin Eisenberger, Emilien Dupont, Po-Sen Huang,
...
[37] Charles Packer, Vivian Fang, Shishir_G Patil, Kevin Lin, Sarah Wooders, and Joseph.E
...
[45] Rich Sutton. The bitter lesson, 2019. URL http://www. incompleteideas. net/Incldeas/Bitter-
Lesson. html, 2019.
13
...
ory systems. arXiv preprint arXivx:2512.18746, 2025.
...
text classification. Each point is one candidatel harness; lines track thel best-so-far. Per-dataset
...
majority of available history rather than conditioning only on thei most recent parent.
Statistic
Value
Files read per iteration (median)
82
Files read per iteration (range) 69-99
File typel breakdown
Harness source code
41%
Execution traces
40%
...
pattern. All text inside the log boxes below is quoted verbatim from thej proposer' 's recorded
reasoning at each iteration (emphasis ours).
Iterations 1-2: promising bugfixes are confounded by prompt edits. The first twoi iterations
...
Smart-waiting (poll pane up to 3x5s) before the next LLM call saves 5--15 turns on
long-running tasks.
...
This candidate is thel best result So far. The important point is not just that iteration 7 wins,
...
Combining two orthogonal fixes
env snapshot (saves early exploration turns) +
...
two representativer endpoints here: Meta-Harness (Draft Verification), thel lowest-context
...
Cold start. If fewer than 51 labeled examples are available, skip the two-stage procedure
...
similar different examples labels
...
d_query.anchored.py. This strongest variant uses a: single larger call built from three parts.
Itl begins with a label primer listing the valid output labels, then constructs a coverage: section
...
exposes the full label space, while the contrastivel block sharpens local decision boundaries
...
This subsection describes thei retrieval harness discovered by Meta-Harness for mathematical
...
Avg metrics
Variant
USPTOT Symptom 1 LawBench 1 Avg T Ctx +
...
35.0
47.5 41.4
Meta-Harness (Relevance + Contrastive) 18.0
90.6
36.0
48.2 43.9
...
context (thousands).
Val vs Test Accuracy by Dataset (gpt-oss-120b)
50
LawBench
Symptom2Disease
USPTO
...
notation. Thel harness does not aggregate outputs across routes: once a routei is selected, only
...
math-aware tokenizer that preserves LaTeX tokens (e.g, frac, (2}) as atomic units. The
...
The discovered TerminalBench-2harness builds on Terminus-KIRA [25], inheriting its native
...
level classification tasks. Thel benchmark tests whether a model can identify and classify
...
setting [24].
TweetEval (Hate) is thel hate-speech subset of the TweetEval benchmark introduced by
...
however, requires operating in a relatively new regime ofl LLM-assisted coding, where the
...
(e.g, few-shot prompting), then construct the search setl by either filtering for examples
...
scores, and execution tracesi in a form that the proposer can query reliably. In practice, this
...
examples. Harnesses proposed during the search should pass this testl before being fully
...
This appendix expands thel brief discussion in Section 2 and situates Meta-Harness relative
...
and provides thej proposer with selective access to prior code, scores, and execution traces
via the filesystem.
AlphaEvolve / OpenEvolve. AlphaEvolve [35] and OpenEvolve [43] evolve code via
LLM-guided mutations with structured feedback: thej proposer receives a program database
